...
3. **後置鏡頭優先技術**:
   - 使用 `facingMode: { ideal: "environment" }` 強制瀏覽器尋找後置相機。
   - 增加關鍵字過濾邏輯（back, rear, 後, 環境），精準命中多鏡頭手機的主鏡頭。
4. **背景預先定位 (Background Warm-up)**:
   - 在進入 QR 頁面時立即啟動 `startBackgroundLocation()`。
   - 利用 `navigator.geolocation` 在背景提取座標，消除掃描成功後才等待 GPS 的延遲。
5. **位置快取技術 (60s Caching)**:
   - 將獲取到的座標暫存 **60 秒**。
   - 對於連續掃描多位同學的情況，第二次掃描後可達到近乎「秒傳」的效果。
6. **懸浮訊息系統 (Floating Overlay)**:
...
## 4. 開發者紀錄 (Changelog)
- **v2.4 (2026-03-21)**: **效能大幅優化**。實作「背景預先定位」與「60 秒座標快取」，將單次掃描反應延遲由 12s 降低至 3s 以內。
- **v2.3 (2026-03-15)**: 修正鏡頭啟動邏輯，實作後置鏡頭 (Rear/Environment) 優先權重，避免預設開啟前置鏡頭。
- **v2.2 (2026-03-15)**: 實作沈浸式全螢幕掃描介面、自動 2.0x 數位變焦以及懸浮彈窗訊息系統。
- **v2.1 (2026-03-15)**: 回溯至 ZXing 穩定版本，並整合手動點名之「自動跳過單一選項」優化邏輯。
- **v2.0 (2026-03-15)**: 核心掃描引擎更換為 ZXing，提升遠距離與複雜 QR Code 識別率。
- **v1.5 (2026-03-13)**: 整合手動點名表單與 QR 簽到模組至單一 HTML 檔案，支援 Tab 切換。
- **v1.0 (2026-03-01)**: 系統初版發布，具備基礎點名與 Google Sheets 資料同步功能。
